...
// ── semente ──────────────────────────────────────────────────
t("semente tem 127 insumos", () => eq(INSUMOS_SEED.length, 127));
...
t("semente tem 110 materiais e 17 prestadores", () => {
  eq(INSUMOS_SEED.filter(x => x.tipo === "material").length, 110);
...
// ── semeadura ────────────────────────────────────────────────
t("semeadura em base vazia cria os 127", () => {
  const r = semearInsumos([], INSUMOS_SEED);
  eq(r.criados, 127);
  eq(r.materiais.length, 127);
});
