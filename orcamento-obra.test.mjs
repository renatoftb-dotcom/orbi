...
teste("estimativa pelos cômodos (tamanho Médio): piso, revestimento, rodapé, soleiras e bancadas", () => {
  ...
  // bancada = fração da parede mais comprida: cozinha 100% de 4 m → 4; WC 50% de 3 m → 1,5
  assert.deepStrictEqual(est.bancadas.map((b) => [b.nome, b.comprimento, b.profundidade]), [["Cozinha", 4, 0.6], ["WC 1", 1.5, 0.5], ["WC 2", 1.5, 0.5]]);
  ...
  assert.deepStrictEqual([g0.composicao[0].tampo, g0.composicao[0].saia, g0.composicao[0].fundo, g0.composicao[0].sapatas], [2.4, 0.2, 0.4, 0.12]);
  assert.strictEqual(g0.qtd, 3.12);
  ...
  assert.deepStrictEqual([g1.composicao[0].saia, g1.composicao[0].fundo, g1.composicao[0].sapatas], [0.4, 0.6, 0.22]); // 4×0,10; 4×0,15; 3×0,6×0,12
  ...
  assert.strictEqual(cp.pisos.bancadas.length, 1); assert.strictEqual(cp.pisos.bancadas[0].comprimento, 4);
...
teste("ilha (cozinha e área de lazer): 1 m menor que a parede maior, laterais de granito, só onde é permitida", () => {
  // Cozinha Médio 4 × 3: bancada 100% da parede maior = 4 m; ilha 4 − 1 = 3 m
  const semIlha = modulo.calcularComodo(modulo.comodoConfig({ tamanhoComodos: "Médio" }, "cozinha"));
  assert.strictEqual(semIlha.bancada.comprimento, 4);
  ...
  assert.strictEqual(c.bancadaM2, 5.28); // bancada 3,12 + ilha 2,16
  ...
  assert.deepStrictEqual(est.bancadas.map((b) => [b.nome, b.comprimento, !!b.ilha]), [["Cozinha", 4, false], ["Cozinha — ilha", 3, true]]);
...
teste("bloco vazio não emite linha nenhuma", () => {
  const out = [];
  modulo.demolicoesRemocoes(projetoReforma({}), out, { materiais: [] });
  modulo.entulhoDaReforma(projetoReforma({}), out, { materiais: [] });
  modulo.execucaoNoExistente(projetoReforma({}), out, { materiais: [] });
  assert.strictEqual(out.length, 0);
});

teste("a reforma inteira entra no orçamento, ordenada e somada", () => {
